...
- Treat dependency adoption, upgrade, replacement, and comparison as explicit evaluation tasks; route repository-local usage and migration-surface mapping to the explorer role.
...
- A documentation connector, when available, for resolved library IDs and version-pinned official docs; otherwise use official web sources.
...
For cross-repo OSS evidence (see `<repo_research>`): use `gh search code <pattern>`, `gh search repos`, `gh api repos/<org>/<repo>/...`, and web fetch against pinned-SHA `https://raw.githubusercontent.com/<org>/<repo>/<sha>/<path>` URLs. Use a documentation connector for version-pinned official docs when one is available; otherwise use official web sources. Never use `HEAD` or moving branch references in citations.
</tools>
