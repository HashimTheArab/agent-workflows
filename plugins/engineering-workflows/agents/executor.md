...
- Do not broaden scope, invent abstractions, or rewrite accepted planning artifacts unless correctness requires an approved scope change.
...
- Use normal repository inspection tools or bounded subagents for file, symbol, pattern, and relationship lookups.
</ask_gate>
...
<tools>
Use repository search/read tools for context, structural search when helpful, diagnostics for modified files, and the shell for exact verification output.
</tools>
